...
The system has effectively one surface mode — dark — with a faint three-step surface ladder (`{colors.canvas}` → `{colors.surface}` → `{colors.surface-elevated}` → `{colors.surface-card}`) carrying cards, in-card panels, and key-cap glyph backgrounds. The signature decorative moment is a **red diagonal-stripe gradient band** across the very top of the home page hero, used as a launch-banner motif behind the headline. Red is the system's signature chromatic voice — it now runs as a full ramp from bright scarlet to deep maroon (`{colors.accent-red-bright}` → `{colors.brand-red}` → `{colors.accent-red-deep}` → `{colors.accent-red-ink}`) — and is paired with a **complementary aqua/teal secondary** (`{colors.accent-aqua}`) sitting at the opposite hue, used as a cool counterpoint in secondary badges, focus states, and a single teal glow band. Beyond these, color in the chrome stays reserved for category accents inside extension and feature illustrations: Hacker News yellow, Slack red, Linear green, info blue.
...
- **Red is the signature chromatic accent, expanded to a full ramp** — `{colors.accent-red-bright}` (#ff8a8a) → `{colors.accent-red}` (#ff6161) → `{colors.brand-red}` (#ff3b3b) → `{colors.accent-red-deep}` (#c8202b) → `{colors.accent-red-ink}` (#7a0f15)
- **Aqua/teal is the complementary secondary** — `{colors.accent-aqua}` (#34d6cd), sitting ~180° opposite the brand red, used for secondary badges, focus rings, and a single teal glow band per page
...
- Signature red diagonal-stripe gradient band at the very top of the hero — three angled stripes deepening across `{colors.hero-stripe-start}` → `{colors.hero-stripe-mid}` → `{colors.hero-stripe-end}`, used once per page maximum, with an optional complementary `{colors.aqua-glow-start}` → `{colors.aqua-glow-end}` teal band as its secondary counterpoint
...
### Secondary — Brand Red & Aqua
> White remains the only **primary** CTA. The red ramp and the aqua family are the system's **secondary** accent vocabulary: they carry secondary badges, focus rings, decorative bands, and emphasis glyphs — never the primary white pill. Red and aqua sit at opposite hues (~0° / ~176°), so a touch of aqua reads as a deliberate cool counterpoint to the dominant red.

- **Brand Red** (`{colors.brand-red}` — `#ff3b3b`) + **Soft** (`{colors.brand-red-soft}` — `rgba(255,59,59,0.15)`): the saturated signature red — secondary-emphasis badges, the "launch"/promo accent, the core of the hero stripe ramp.
- **Brand Red Pressed** (`{colors.brand-red-pressed}` — `#e22d2d`): pressed-state for any red secondary control.
- **Accent Red Bright** (`{colors.accent-red-bright}` — `#ff8a8a`): the lightest red tint — red glyphs, highlight text on dark, and the top of the stripe ramp.
- **Accent Red** (`{colors.accent-red}` — `#ff6161`) + **Soft** (`{colors.accent-red-soft}` — `rgba(255,97,97,0.15)`): mid red — destructive/error indicator + Slack/Apple category accent in extension illustrations.
- **Accent Red Deep** (`{colors.accent-red-deep}` — `#c8202b`): crimson — gradient mid-stop and high-emphasis red on lighter surfaces.
- **Accent Red Ink** (`{colors.accent-red-ink}` — `#7a0f15`): darkest maroon — the deep end of the stripe gradient and shadowy red washes.
- **Accent Aqua** (`{colors.accent-aqua}` — `#34d6cd`) + **Soft** (`{colors.accent-aqua-soft}` — `rgba(52,214,205,0.15)`): the complementary secondary — secondary badge color, focus-ring tint, "AI"/feature accent, the core of the aqua glow band.
- **Accent Aqua Pressed** (`{colors.accent-aqua-pressed}` — `#28bdb4`): pressed-state for any aqua secondary control.
- **Accent Aqua Bright** (`{colors.accent-aqua-bright}` — `#7df0ea`): the lightest aqua tint — aqua glyphs and highlight text on dark.
- **Accent Teal Deep** (`{colors.accent-teal-deep}` — `#0d8f88`): deep teal — gradient end-stop and high-emphasis aqua.
- **Accent Teal Ink** (`{colors.accent-teal-ink}` — `#07514d`): darkest teal wash — the deep end of the aqua glow band.
...
- **Accent Red** (`{colors.accent-red}` — `#ff6161`) + **Soft** (`{colors.accent-red-soft}` — `rgba(255,97,97,0.15)`): destructive/error indicator + Slack/Apple category accent in extension illustrations. (Part of the full red ramp documented under Secondary.)
...
- **Accent Aqua** (`{colors.accent-aqua}` — `#34d6cd`) + **Soft** (`{colors.accent-aqua-soft}` — `rgba(52,214,205,0.15)`): the secondary/"active-AI" semantic — focus rings and feature accents. (Part of the aqua family documented under Secondary.)

### Brand Gradient
- **Hero Stripe Gradient** — three diagonal red stripes layered across the very top of the home page hero, deepening across a three-stop red ramp: `{colors.hero-stripe-start}` (`#ff5757`) → `{colors.hero-stripe-mid}` (`#c8202b`) → `{colors.hero-stripe-end}` (`#7a0f15`). The system's signature chromatic gradient on chrome — used once per page maximum and reserved for hero launch-banner moments. The added mid-stop gives the band a richer, deeper-red falloff than a two-stop fade.
- **Aqua Glow Gradient** — the complementary secondary band: a single soft teal wash fading from `{colors.aqua-glow-start}` (`#34d6cd`) → `{colors.aqua-glow-end}` (`#07514d`). Used as the cool counterpoint to the red hero — at most once per page, typically behind an AI/feature headline or as a faint footer echo opposite the red stripe. Never co-located in the same band as the red stripe.
...
### Decorative Depth
Depth comes from product imagery and the two gradient bands:
- **Hero stripe gradient** — three diagonal red stripes (`{colors.hero-stripe-start}` → `{colors.hero-stripe-mid}` → `{colors.hero-stripe-end}`) layered across the home-page hero band, evoking a launch-banner / motion-blur effect. The system's signature decorative moment.
- **Aqua glow gradient** — a single soft teal wash (`{colors.aqua-glow-start}` → `{colors.aqua-glow-end}`) used at most once per page as the cool secondary counterpoint to the red hero.
...
- **Aqua glow gradient** — full-bleed wash with no aspect ratio.
...
**`button-secondary-red`** — red-accented secondary button
- Background `{colors.brand-red-soft}`, text `{colors.accent-red-bright}`, 1px solid `transparent`, type `{typography.button-md}`, padding `8px 16px`, height ~36px, rounded `{rounded.md}`.
- Secondary-emphasis red action — promo / "launch" CTAs that must NOT compete with the white primary pill. Pressed: background lifts toward `{colors.brand-red}` at 22% with text `{colors.on-dark}`.

**`button-secondary-aqua`** — aqua-accented secondary button
- Background `{colors.accent-aqua-soft}`, text `{colors.accent-aqua-bright}`, type `{typography.button-md}`, padding `8px 16px`, height ~36px, rounded `{rounded.md}`.
- The cool counterpart to `button-secondary-red` — "Try the AI" / feature secondary actions. Pressed: background lifts toward `{colors.accent-aqua}` at 22% with text `{colors.on-dark}`.
...
**`badge-red-soft`** — translucent red secondary chip
- Background `{colors.brand-red-soft}`, text `{colors.accent-red-bright}`, type `{typography.caption-sm}`, padding `2px 8px`, rounded `{rounded.xs}`.
- Secondary-emphasis "Launch" / "Hot" / "New" promo tag — the red counterpart to `badge-info-soft`.

**`badge-aqua-soft`** — translucent aqua secondary chip
- Background `{colors.accent-aqua-soft}`, text `{colors.accent-aqua-bright}`, type `{typography.caption-sm}`, padding `2px 8px`, rounded `{rounded.xs}`.
- The cool counterpart to `badge-red-soft` — "AI" / "Beta" feature tag.
...
- Focused: same surface; 1px border becomes `{colors.accent-aqua}` at full or `{colors.accent-aqua-soft}` for a softer ring — the secondary aqua now carries the focus tint (replacing the prior monochrome `{colors.hairline-strong}` brighten where a colored focus cue is wanted; the hairline-strong brighten remains acceptable on chrome that must stay fully monochrome).
...
- Sits at the top of the store page hero with a magnifier icon at the left and "Search the store..." placeholder. Slightly taller than the standard `text-input`. Focus ring follows `text-input-focused` (`{colors.accent-aqua}`).
...
- Same chrome but background flips to `{colors.surface-elevated}` (one notch lighter), with a 1px top edge in `{colors.brand-red}` at 40% (a thin red accent rule) marking the featured tier — the only chromatic cue distinguishing it from the surrounding cards.

**`hero-stripe-band`** — home-page hero with red stripe gradient
- Background `{colors.canvas}` with three diagonal red stripes layered across the top half (`{colors.hero-stripe-start}` → `{colors.hero-stripe-mid}` → `{colors.hero-stripe-end}`).
...
**`aqua-glow-band`** — secondary feature/AI band with teal glow
- Background `{colors.canvas}` with a single soft teal wash across the top (`{colors.aqua-glow-start}` → `{colors.aqua-glow-end}`).
- Padding `{spacing.section}` 96px vertical / 48px horizontal, rounded `{rounded.none}`.
- The cool counterpart to `hero-stripe-band` — carries a feature/AI headline in `{typography.display-lg}`. Used at most once per page and never in the same band as the red stripe.
...
- The very top of the footer band carries a faint gradient echo — either the red stripe repeat (`{colors.hero-stripe-start}` → `{colors.hero-stripe-end}`) or, when the hero already spent the red moment, the complementary `{colors.aqua-glow-start}` → `{colors.aqua-glow-end}` teal echo. One echo only, never both.

### Inline

**`link-inline`** — body-prose anchor link
- `{colors.on-dark}` text with no underline by default; underlines on focus. Inline body links are full-white rather than a tinted accent color, which keeps the dark canvas tonally pure. (Aqua and red stay reserved for badges, focus, and decorative bands — not inline link text.)

## Do's and Don'ts

### Do
- Render the entire site in one continuous dark mode. There is no light variant in the system.
- Use `{colors.primary}` (white pill) for every **primary** CTA. White IS the brand action; red and aqua are secondary accents, never the primary pill.
...
- Reach for the red ramp (`{colors.accent-red-bright}` → `{colors.brand-red}` → `{colors.accent-red-deep}` → `{colors.accent-red-ink}`) for the signature decorative band and secondary red badges/buttons — red is the dominant accent voice.
- Use the complementary aqua family (`{colors.accent-aqua}` and tints) as the cool secondary counterpoint — focus rings, secondary "AI"/feature badges, and a single teal glow band. Keep it scarcer than red.
- Reserve the `{colors.hero-stripe-start}` → `{colors.hero-stripe-mid}` → `{colors.hero-stripe-end}` red gradient for the hero band exactly once per page, and the `{colors.aqua-glow-start}` → `{colors.aqua-glow-end}` teal band for at most one secondary moment. Never repeat either deeper in the page.
- Use saturated category accents (`{colors.accent-yellow}`, `{colors.accent-red}`, `{colors.accent-green}`, `{colors.accent-blue}`) inside extension and feature illustrations.
...
- Don't replace `{colors.primary}` (white) with red, aqua, or any tinted accent for the **primary** CTA. Pure white is the brand primary; red/aqua are secondary only.
- Don't let aqua out-shout red. Red is the dominant accent; aqua is the restrained complement — roughly one aqua moment for every red one, and never two saturated bands in the same fold.
- Don't co-locate the red stripe and the aqua glow in the same band, or repeat either gradient outside its one allowed moment. The one-band-each rule is the system's restraint.
- Don't scatter red or aqua across body text or chrome surfaces beyond their documented roles (badges, focus, buttons-secondary, decorative bands). Inline links stay white.
...
- **Aqua glow gradient** stays at the top of its secondary band at every breakpoint.
...
6. Keep `{colors.primary}` (white CTA pill) scarce per viewport — at most one solid white pill per fold. Keep red the dominant accent and aqua the scarcer complement — at most one saturated band of each per page.
7. When introducing a new component, ask whether it can be expressed with the existing surface-ladder + 8px-radius + ss03-Inter + red/aqua-accent vocabulary before adding new tokens. The system's strength is that it almost never needs new ones.
...
- **Red/aqua secondary is an extension**, not an observed Raycast surface — the original capture had only the single red stripe and white CTA. The expanded red ramp and the complementary aqua family are a deliberate addition to the system, not documented Raycast chrome.
